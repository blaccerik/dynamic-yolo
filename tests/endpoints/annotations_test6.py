...
class AnnotationsTest6(TestCase):
    ...
    def test1(self):
        ...
        json_data = {'uploader': "erik", 'keep': "human"}
        response = self.client.post(f"/annotations/{ae5.id}", json=json_data)
        assert response.status_code == 400

        json_data = {'uploader': "erik", 'keep': "model"}
        response = self.client.post(f"/annotations/{ae6.id}", json=json_data)
        assert response.status_code == 400

        json_data = {'uploader': "erik", 'keep': "both"}
        response = self.client.post(f"/annotations/{ae7.id}", json=json_data)
        assert response.status_code == 400
        ...
        assert Annotation.query.get(a2.id) is not None
        assert Annotation.query.get(a3.id) is None
        assert Annotation.query.get(a4.id) is None
        ...
        assert Annotation.query.get(a4.id) is None
        assert Annotation.query.get(a5.id) is not None
        assert Annotation.query.get(a5.id).annotator_id == a.id
