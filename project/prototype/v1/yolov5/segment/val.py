...
def run(
    ...
    if training:  # called by train.py
        ...
        nm = de_parallel(model).db_model[-1].nm  # number of masks
        ...
        nm = de_parallel(model).db_model.db_model[-1].nm if isinstance(model, SegmentationModel) else 32  # number of masks
        if engine:
