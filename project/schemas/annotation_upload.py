from marshmallow import Schema, fields, validates_schema, ValidationError

from project.models.annotator import Annotator
from project.models.image import Image
from project.models.project import Project
from project.models.project_settings import ProjectSettings


class AnnotationUploadSchema(Schema):
    x_center = fields.Float(required=True)
    y_center = fields.Float(required=True)
    width = fields.Float(required=True)
    height = fields.Float(required=True)
    class_id = fields.Integer(required=True)
    project_id = fields.Integer(required=True)
    image_id = fields.Integer(required=True)
    uploader = fields.String(required=True)

    @validates_schema
    def validate_fields(self, data, **kwargs):
        if "class_id" in data:
            project_id = data['project_id']
            project = Project.query.get(project_id)
            if project:
                project_settings = ProjectSettings.query.filter_by(id=project_id).first()
                max_class_nr = project_settings.max_class_nr
                if data["class_id"] <= 0 or data["class_id"] > max_class_nr - 1:
                    raise ValidationError(
                        f"Class ID must be between 0 and {max_class_nr - 1}",
                        "class_id"
                    )
            else:
                raise ValidationError(f"Project with ID: {project_id}, was not found!")
        if "image_id" in data:
            image_id = data['image_id']
            image = Image.query.get(image_id)
            if not image:
                raise ValidationError(f"Image with the ID: {image_id}, is not in the database!",
                                      "image_id")

        if "uploader" in data:
            uploader = data['uploader']
            user = Annotator.query.filter(Annotator.name.like(uploader)).first()
            if not user:
                raise ValidationError(f"User with name: '{uploader}', not in database!",
                                      "uploader")

        self.in_range("x_center", data)
        self.in_range("y_center", data)
        self.in_range("width", data)
        self.in_range("height", data)

    def in_range(self, key, data):
        if key in data:
            if data[key] < 0 or data[key] > 1:
                raise ValidationError("Must be between 0 and 1", key)
